...
typedef struct {
    char name[MAX_NICK];
    int members[MAX_CLIENTS]; // indices des clients dans clients[]
    int member_count;
} Salon;
...
int find_salon(const char *name) {
    for (int i=0;i<MAX_SALONS;i++) {
        if (strcmp(salons[i].name,name)==0) return i;
    }
    return -1;
}
...
int add_client_to_salon(int client_index, const char *salon_name) {
    ...
    if (idx < 0) {
        // Salon vide ?
        for (int i=0;i<MAX_SALONS;i++) {
            if (salons[i].name[0]=='\0') {
                strcpy(salons[i].name,salon_name);
                idx = i;
                break;
            }
        }
        if (idx<0) return -1; // plus de place
    }

    // Vérifier si déjà membre
    for (int j=0;j<salons[idx].member_count;j++)
        if (salons[idx].members[j]==client_index) return idx;
...
void remove_client_from_salon(int client_index) {
    ...
    int new_count=0;
    for (int j=0;j<salons[idx].member_count;j++) {
        if (salons[idx].members[j]!=client_index)
            salons[idx].members[new_count++] = salons[idx].members[j];
    }
    salons[idx].member_count = new_count;
    clients[client_index].salon[0]='\0';

    if (salons[idx].member_count==0) {
        salons[idx].name[0]='\0'; // supprimer salon vide
    }
}
...
void broadcast(int sender_index, const char *msg) {
    for (int i=0;i<MAX_CLIENTS;i++) {
        if (i!=sender_index && clients[i].socket>0 && clients[i].is_registered) {
...
int main(int argc, char *argv[]) {
    ...
    init_clients();
    init_salons();
    printf("Serveur Chat V4 en écoute sur le port %d...\n",port);
    ...
    while (1) {
        ...
        for(int i=0;i<MAX_CLIENTS;i++) {
            ...
            if(sd>0 && FD_ISSET(sd,&readfds)) {
                ...
                if(bytes<=0) {
                    remove_client_from_salon(i);
                    close(sd); clients[i].socket=0; clients[i].is_registered=0; clients[i].nickname[0]='\0';
                    continue;
                }
                ...
                if(strcmp(msg,"/quit")==0) {
                    send_to_client(i,"[Server] : You will be terminated");
                    remove_client_from_salon(i);
                    close(sd); clients[i].socket=0; clients[i].is_registered=0; clients[i].nickname[0]='\0';
                    continue;
                }
                ...
                if(strncmp(msg,"/nick ",6)==0) {
                    char *nick=msg+6; strncpy(clients[i].nickname,nick,MAX_NICK-1); clients[i].is_registered=1;
                    send_to_client(i,"[Server] : Pseudo enregistré");
                    continue;
                }

                // /broadcast
                if(strncmp(msg,"/broadcast ",11)==0) {
                    char text[BUFFER_SIZE]; snprintf(text,BUFFER_SIZE,"[%s]: %s",clients[i].nickname,msg+11);
                    broadcast(i,text); continue;
                }

                // /msg <nick> <message>
                if(strncmp(msg,"/msg ",5)==0) {
                    char target[MAX_NICK]; char text[BUFFER_SIZE];
                    sscanf(msg+5,"%s %[^\n]",target,text);
                    int found=0;
                    for(int j=0;j<MAX_CLIENTS;j++) {
                        if(clients[j].is_registered && strcmp(clients[j].nickname,target)==0) {
                            char tmp[BUFFER_SIZE]; snprintf(tmp,BUFFER_SIZE,"[Private][%s]: %s",clients[i].nickname,text);
                            send_to_client(j,tmp); found=1; break;
                        }
                    }
                    if(!found) send_to_client(i,"[Server] : Utilisateur introuvable"); 
                    continue;
                }

                // /create <salon>
                if(strncmp(msg,"/create ",8)==0) {
                    char *salon_name=msg+8;
                    if(find_salon(salon_name)>=0) { send_to_client(i,"[Server] : Salon existe déjà"); continue; }
                    if(add_client_to_salon(i,salon_name)<0) { send_to_client(i,"[Server] : Impossible de créer salon"); continue; }
                    send_to_client(i,"[Server] : Salon créé et rejoint"); continue;
                }

                // /join <salon>
                if(strncmp(msg,"/join ",6)==0) {
                    char *salon_name=msg+6;
                    if(add_client_to_salon(i,salon_name)<0) { send_to_client(i,"[Server] : Impossible de rejoindre salon"); continue; }
                    send_to_client(i,"[Server] : Salon rejoint"); continue;
                }
                ...
                // /salonmsg
                if(strncmp(msg,"/salonmsg ",10)==0) {
                    char text[BUFFER_SIZE]; snprintf(text,BUFFER_SIZE,"[%s][Salon]: %s",clients[i].nickname,msg+10);
                    send_salon(i,text); continue;
                }
