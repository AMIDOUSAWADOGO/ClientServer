...
void aes_encrypt(const unsigned char *input, unsigned char *output, int len){
    AES_KEY key; AES_set_encrypt_key((unsigned char*)AES_KEY_STR,256,&key);
...
void aes_decrypt(const unsigned char *input, unsigned char *output, int len){
    AES_KEY key; AES_set_decrypt_key((unsigned char*)AES_KEY_STR,256,&key);
    for(int i=0;i<len;i+=AES_BLOCK_SIZE) AES_decrypt(input+i,output+i,&key);
}

int main(int argc,char *argv[]){
    if(argc!=3){ printf("Usage: %s <port> <server_ip>\n",argv[0]); exit(1);}
    int port=atoi(argv[1]); char *ip=argv[2];
    int sock=socket(AF_INET,SOCK_STREAM,0);
    if(sock<0){ perror("socket"); exit(1);}
    struct sockaddr_in server; server.sin_family=AF_INET; server.sin_port=htons(port); inet_pton(AF_INET,ip,&server.sin_addr);
    ...
    while(1){
        ...
        if(FD_ISSET(STDIN_FILENO,&readfds)){
            ...
            if(strncmp(input,"/receivefile ",13)==0){ char filename[BUFFER_SIZE]; sscanf(input+13,"%s",filename); FILE *f=fopen(filename,"wb"); if(!f){printf("Impossible de créer fichier.\n"); continue;} int n=recv(sock,buffer,BUFFER_SIZE,0); aes_decrypt(buffer,(unsigned char*)buffer,n); fwrite(buffer,1,n,f); fclose(f); printf("Fichier reçu: %s\n",filename); continue;}
            
            unsigned char enc[BUFFER_SIZE]={0}; int aes_len=((strlen(input)+AES_BLOCK_SIZE-1)/AES_BLOCK_SIZE)*AES_BLOCK_SIZE;
            aes_encrypt((unsigned char*)input,enc,aes_len); send(sock,enc,aes_len,0);
